...
namespace Drupal\ocha_hid_contacts\Plugin\Field\FieldFormatter;
...
/**
 * Plugin implementation of the 'ocha_hid_contacts' formatter.
 *
 * @FieldFormatter (
 *   id = "ocha_hid_contacts_default",
 *   label = @Translation("Ocha HID Contacts formatter"),
 *   field_types = {
 *     "ocha_hid_contacts"
 *   }
 * )
 */
class OchaHidContactsDefaultFormatter extends OchaIntegrationsDefaultFormatter {
    ...
    if ($items->count()) {
      ...
      foreach ($items as $delta => $item) {
        ...
        if ($this->getSetting('output') != 'label') {
          // Get item data.
          $data = ocha_hid_contacts_get_item($value, $langcode);
